...
module.exports = function(configuration) {
  function send(req, extended) {
    ...
    if (
      ...
      return Promise.reject(
        new TypeError('Requires country_code, store_id, api_token')
      );
    }
  ...
  var pay = function(args) {
    ...
    var expdate = normalizeExpiry(
      configuration.expiryFormat,
      cleanse(args.expiry, true)
    );
    ...
    var order_id =
      args.order_id ||
      cleanse(configuration.app_name, true) + '-Purchase-' + suffix;
    var cust_id = args.cust_id || 'customer-' + suffix;
    var dynamic_descriptor =
      args.description || args.dynamic_descriptor || 'purchase';
    var purchase = {
      type: 'purchase',
      cust_id,
      order_id,
    ...
    return $q.fcall(function() {
      ...
      return send(purchase).then(function(result) {
        ...
        var approved =
          !status.timeout &&
          (code == '00' || code ? parseInt(code) < 50 : false);
        return $q.fcall(function() {
          if (approved) {
            return status;
          } else {
